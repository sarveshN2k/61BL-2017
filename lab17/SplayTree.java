...
public class SplayTree<K extends Comparable<K>, V> {
    ...
    void rotateRight(TreeNode<K, V> node) {
        if (node == null || node.parent == null || node.parent.left != node) {
            ...
            TreeNode<K, V> oriRoot = root;
            TreeNode<K, V> oriRight = root.left.right;
            root = root.left;
            root.right = oriRoot;
            root.right.left = oriRight;
            root.parent = null;
            oriRoot.parent = root;
            if (oriRight != null) {
                oriRight.parent = oriRoot;
            }
        } else {
            TreeNode<K, V> grandParent = node.parent.parent;
            TreeNode<K, V> origParent = node.parent;
            TreeNode<K, V> origRight = node.right;
            node.parent = grandParent;
            if (grandParent.right == origParent){
                grandParent.right = node;
            } else {
                grandParent.left = node;
            }
            node.right = origParent;
            origParent.parent = node;
            origParent.left = origRight;
            if (origRight != null) {
                origRight.parent = origParent;
            }
        }
    }
    ...
    void rotateLeft(TreeNode<K, V> node) {
        if (node == null || node.parent == null || node.parent.right != node) {
            ...
            TreeNode<K, V> oriRoot = root;
            TreeNode<K, V> oriLeft = root.right.left;
            root = root.right;
            root.left = oriRoot;
            root.left.right = oriLeft;
            root.parent = null;
            oriRoot.parent = root;
            if (oriLeft != null) {
                oriLeft.parent = oriRoot;
            }
        } else {
            TreeNode<K, V> grandParent = node.parent.parent;
            TreeNode<K, V> origParent = node.parent;
            TreeNode<K, V> origLeft = node.left;
            node.parent = grandParent;
            if (grandParent.left == origParent){
                grandParent.left = node;
            } else {
                grandParent.right = node;
            }
            node.left = origParent;
            origParent.parent = node;
            origParent.right = origLeft;
            if (origLeft != null) {
                origLeft.parent = origParent;
            }
        }
    }
    ...
    private void zigZig(TreeNode<K, V> node) {
        // YOUR CODE HERE
        TreeNode<K, V> grandParent = node.parent.parent;
        if (grandParent.left != null && node == grandParent.left.left) {
            rotateRight(node.parent);
            rotateRight(node);
        } else if (grandParent.right != null && node == grandParent.right.right) {
