...
public class DistributionSorts {
    ...
    public static void countingSort(int[] arr) {
        // TODO your code here!
        //Much help from geeksforgeeks and cs.usfca
    ...
    public static void csod(int[] arr, int digit) {
    ...
    private static void countingSortOnDigit(int[] arr, int digit) {
        ...
        int[][] valMatrix = new int[10][arr.length];
        int[] counts = new int[10];

        for (int i = 0; i < arr.length; i++) {
            int index = (arr[i] / modulo) % 10;
            valMatrix[index][counts[index]] = arr[i];
            counts[index]++;
        }
        int i = 0;
        for (int index = 0; index < valMatrix.length; index++) {
            for (int count = 0; count < counts[index]; count++) {
                toReturn[i] = valMatrix[index][count];
                i++;
            }
        }
